...
def create_and_save_plot(data, ticker, period, filename=None, style='default', std_dev=None):
    ...
    if 'Date' in data:
        if  pd.api.types.is_datetime64_any_dtype(data.index):
            dates = data.index.to_numpy().nditer()
        ...
        if not pd.api.types.is_datetime64_any_dtype(data['Date']):
            data['Date'] = pd.to_datetime(data['Date'].nditer())
...
def plot_close_average_bokeh(data):
        # Проверяем, есть ли колонка 'Close' в DataFrame
    ...
    p = figure(title='График цен закрытия', x_axis_label='Дата', y_axis_label='Цена закрытия',
                   x_axis_type='datetime')
    p.line('x', 'y', source=source, line_width=2)

    # Добавляем линию среднего значения
    avg_line = Span(location=average_close, dimension='width', line_color='red', line_dash='dashed', line_width=2)
    p.add_layout(avg_line)

    # Показываем график
    output_notebook()  # Используйте это для Jupyter Notebook
    show(p)
