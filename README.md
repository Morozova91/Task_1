...
- notify_if_strong_fluctuations(data, threshold): Вычисляет максимальное и минимальное значения цены закрытия и сравнивать разницу с заданным порогом.
- compute_macd(data, short_window=12, long_window=26, signal_window=9): Вычисляет индикатор MACD и сигнальную линию на основе цен закрытия.
- calculate_rsi(data, period=14): Рассчитывает индекс относительной силы (RSI) на основе цен закрытия.
2. main.py:
...
- Также в create_and_save_plot были добавлены данные:
- График RSI
- График цен
- График MACD
- plt.style.use: Возможность пользователю выбрать стиль оформления графика.
- export_to_csv(data, filename): Экспортирует данные в CSV файл.
...
Пример выполненной работы с выбором стиля графика:

![2025-01-20_11-02-56](https://github.com/user-attachments/assets/b739a411-ce7a-40a9-a08a-e7a666a33848)
